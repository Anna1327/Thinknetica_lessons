...
class RailRoad
  ...
  def check_attempts(number, error)
    number += 1
    if number < 5
      puts "Введите валидные данные:#{error.message}"
    else
  ...
  def create_carriage
    ...
    puts carriage_type == 1 ? 'Введите объем вагона' : 'Введите количество мест в вагоне'
  ...
  def create_route
    ...
    stations = add_station_to_stations(name_of_first_station, name_of_last_station)
  ...
  def take_volume(carriage)
    if carriage.type == 'cargo'
      take_volume_cargo(carriage)
    else
      take_volume_passenger(carriage)
    end
  end

  # Display a list of wagons by the train
  def show_train_carriages
    ...
    train.block_carriages do |carriage|
      print " Тип вагона: #{carriage.type},"
      print "свободные места: #{carriage.free_place},"
      print "занятые места: #{carriage.used_place}"
    end
  end

  def check_station(name)
    station = find_station(name)
    puts "Станции #{name} не существует" unless station
    puts "Нет поездов на станции #{name}" if station.trains.size.zero?
    return false if !station || station.trains.size.zero?
  end

  # Show a list of trains at a station
  def show_station_trains
    return unless check_station(name_of_station)

    station.block_trains do |train|
      puts "Тип поезда: #{train.type}, номер поезда: \##{train.number}, количество вагонов: #{train.carriages.size}"
    end
  end
